...
func TestServiceManagedRecipeLifecycle(t *testing.T) {
  ...
  "eval_trace":[{"decision_name":"decision-a","matched":true}]
	...
	if descriptor.Digests.Recipe == "" || strings.Contains(descriptor.README, dir) {
...
func TestServiceMessagesRunPlanPreservesFixture(t *testing.T) {
	...
	plan, err := service.RunPlan("lane-a", "messages-a")
	if err != nil {
		t.Fatalf("RunPlan(): %v", err)
	}
	if len(plan.Messages) != 3 || len(plan.Tools) != 0 {
		t.Fatalf("messages plan = %#v", plan)
	}
	if got := plan.Messages[2]["content"]; got != "try again" {
