...
function RecipeModelPool({ recipe }: { recipe: RecipeConfig | null }) {
  if (!recipe) return <div className={pageStyles.poolDisclosure}>Recipe not found.</div>
    ...
    <div className={pageStyles.poolDisclosure}>
      <div className={pageStyles.poolDisclosureHeader}>
        ...
        <span className={pageStyles.recipeBadge}>
          {collectRecipeTargetModels(recipe).length} configured models
        </span>
      </div>
      {decisions.length > 0 ? (
            ...
            <article key={decision.name} className={pageStyles.poolDecisionCard}>
              ...
              <div className={pageStyles.poolModelList}>
                {(decision.modelRefs ?? []).map((reference) => (
                  <div key={`${decision.name}-${reference.model}`} className={pageStyles.poolModel}>
        ...
        {filtered.map((entrypoint) => {
          ...
          return (
            <article key={key} className={pageStyles.portfolioItem}>
              ...
              {isExpanded ? <RecipeModelPool recipe={recipe} /> : null}
            </article>
          )
        })}
        {filtered.length === 0 ? (
