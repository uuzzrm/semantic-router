...
const editableContent = (message: RecipeProbeMessage | undefined): string => {
  if (!message) return ''
...
export const preparePlaygroundInvocation = (
  ...
  return {
    ...
    prompt: terminalIsUser ? editableContent(messages[messageIndex]) : '',
    ...(invocation.model ? { model: invocation.model } : {}),
    history: terminalIsUser ? messages.slice(0, messageIndex) : messages,
    messages,
    editableMessageIndex: messageIndex,
    editable: invocation.editable && terminalIsUser,
...
export const materializeEditedProbeRequest = (
  ...
  const messages = prepared.messages.map((message, index) =>
    index === prepared.editableMessageIndex
      ? {
          ...message,
          role: 'user',
          content: prompt,
...
export const toPlaygroundHistory = (
  ...
  history.forEach((message) => {
    if (message.role === 'tool') {
      ...
      if (!owner) {
        result.push({
          id: generateId(),
          role: 'system',
          content: `Tool result${callID ? ` (${callID})` : ''}: ${editableContent(message)}`,
          timestamp: new Date(),
        })
        return
      }
    ...
    const next: Message = {
      id: generateId(),
      role: message.role,
      content: editableContent(message),
      timestamp: new Date(),
