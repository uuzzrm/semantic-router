...
The policy expresses feedback recovery with router-native primitives:
...
`probes.yaml` mixes two probe shapes:
...
These are the maintained probe families in `probes.yaml`.
...
go run ./cmd/dsl decompile \
  -o ../../config/recipes/feedback/recipe.dsl \
  ../../config/recipes/feedback/config.yaml
go run ./cmd/dsl validate ../../config/recipes/feedback/recipe.dsl
```
...
python3 tools/agent/scripts/router_calibration_loop.py eval \
  ...
  --probes config/recipes/feedback/probes.yaml
```
